...
- [x] **T-006.2** Write `pipeline/03_pull_playbyplay.py` that pulls play-by-play data per unique `GAME_ID` (score fields included), with the same retry/backoff/delay approach as T-005 — **superseded 2026-07-13:** originally used `playbyplayv2`, whose `SCOREMARGIN`/`SCORE` columns give the score state directly per event. **Real bug found running this for real against live data:** `playbyplayv2` is broken on the current live API — every single game failed with `KeyError('resultSet')` after all retries, a known/documented `nba_api` issue (confirmed via web search: the endpoint returns a response shape the library can no longer parse; upstream fix is `playbyplayv3`). Migrated to `playbyplayv3`, which uses `scoreHome`/`scoreAway` running-score columns instead of a single `SCOREMARGIN` string — see T-011.3 for the score-margin computation change. **A second, related bug was caught in the same pass:** the original code read `endpoint.get_data_frames()[0]`, which is actually the `AvailableVideo` dataset (a single video-flag column), not `PlayByPlay` (index 1) — invisible in the earlier mocked unit test since the mock fully controlled `get_data_frames()`'s return value and never exercised the real indexing. Fixed to use the named `endpoint.play_by_play.get_data_frame()` accessor instead of a magic index, for both correctness and to prevent silent regression.
...
- **Verification note:** originally logic-verified only with a mocked `PlayByPlayV2` response (sandbox network constraint). **2026-07-13: this task's real gap was exactly the one flagged** — running against the live API for the first time (by the user, on their own machine) surfaced the v2/v3 incompatibility and the dataset-indexing bug above, neither of which mocked testing could have caught. Both are now fixed and re-verified with a `playbyplayv3`-shaped mock; still needs a live rerun to fully confirm end-to-end (see T-005.6's same caveat).
...
- [x] **T-011.2** For each shot, join on `GAME_ID` + period + game clock to find the score state immediately prior to that shot event — **deviation:** joined on the exact `GAME_ID` + `GAME_EVENT_ID`/`actionNumber` match instead of a fuzzy clock-time join. `shotchartdetail`'s `GAME_EVENT_ID` and `playbyplayv3`'s `actionNumber` refer to the same underlying per-game event log, so an exact-ID `merge_asof` (backward, strictly-before) is both simpler and more precise than matching on period+clock string. (Originally matched against `playbyplayv2`'s `EVENTNUM` — see the T-006.2 superseded note for why this moved to v3.)
- [x] **T-011.3** Compute `score_margin` (shooting team's score minus opponent's score at that moment) — **superseded 2026-07-13:** originally read `playbyplayv2`'s own `SCOREMARGIN` field directly (forward-filled per game). Since T-006.2 migrated to `playbyplayv3` (v2 is broken on the live API — see that note), `score_margin` is now computed directly as `scoreHome - scoreAway` from v3's two running-score columns (same forward-fill-per-game approach for gaps), which is arguably more robust than parsing a signed string anyway.
...
- **Verification note:** the `merge_asof`(backward, strictly-before) join logic was unit-tested directly with a small synthetic play-by-play sequence to confirm it picks up the state from the *prior* event, not the shot's own event (see todo.md's T-009 verification note for the broader sandbox network constraint). **2026-07-13:** re-verified against a `playbyplayv3`-shaped mock (`scoreHome`/`scoreAway` columns) after the v2→v3 migration — same test, same result (correctly picks up the prior event's score state, not the shot's own).
